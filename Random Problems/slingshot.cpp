...
public:
	...
	void update(int idx, ll v, int l = 0, int r = 1e9+5) {
		if(l == r) {
			val = v;
			return;
		}
		...
		val = min(c[0] ? c[0] -> val : INF_T, c[1] ? c[1] -> val : INF_T);
	}
	ll query(int a, int b, int l = 0, int r = 1e9+5) {
		if(l >= a && r <= b)
			return val;
		if(l > b || r < a)
			return INF_T;
		int mid = getMid(l, r);
		ll res = (c[0] ? c[0] -> query(a, b, l, mid) : INF_T);
		res = min(res, c[1] ? c[1] -> query(a, b, mid+1, r) : INF_T);
		return res;
	}
};
...
bool cmp(const sling* a, const sling* o)  {
	return tie(a->x, a->y) < tie(o->x, o->y);
}
Node seg[8];
int main() {
	FAST_IO;
	...
	F0R(i, 2) {
		F0R(j, 4) {
			for(auto cSl: ss[i]) {
				if(cSl->z == -1) {
					if(j == 0)
						cSl->ans = min({cSl->ans, cSl->y - cSl->x, cSl->y + cSl->x + seg[i*4+j].query(cSl->x, cSl->y)});
					else if(j == 1)
						cSl->ans = min({cSl->ans, cSl->y - cSl->x, -cSl->y + cSl->x + seg[i*4+j].query(cSl->y, INF)});
					else if(j == 2)
						cSl->ans = min({cSl->ans, cSl->y - cSl->x, cSl->y + -cSl->x + seg[i*4+j].query(cSl->x, cSl->y-1)});
					else
						cSl->ans = min({cSl->ans, cSl->y - cSl->x, -cSl->y + -cSl->x + seg[i*4+j].query(cSl->y, INF)});
				} else {
					if(j == 0)
						seg[i*4+j].update(cSl->y, -cSl->x-cSl->y+cSl->z);
					if(j == 1)
						seg[i*4+j].update(cSl->y, -cSl->x+cSl->y+cSl->z);
					if(j == 2)
						seg[i*4+j].update(cSl->y, cSl->x-cSl->y+cSl->z);
